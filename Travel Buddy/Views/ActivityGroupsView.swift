    ...
    struct GroupRowView: View {
        ...
        private var actionBackground: Color {
            if hasJoined { return Color.gray.opacity(0.28) }
            if group.isFull { return Color.gray.opacity(0.28) }
            return AppColors.accent
        }

        private var actionForeground: Color {
            if hasJoined { return .gray }
        ...
        private var leftSlots: [Slot] {
            Array(slots.prefix(3))
        }

        private var middleSlots: [Slot] {
            Array(slots.dropFirst(3).prefix(2))
        }
